...
describe("mux assets list command", () => {
  ...
  describe("Optional flags", () => {
    ...
    test("has --upload-id flag for filtering", () => {
      const uploadIdOption = listCommand.getOptions().find((opt) => opt.name === "upload-id");
      expect(uploadIdOption).toBeDefined();
    });

    test("has --live-stream-id flag for filtering", () => {
      const liveStreamIdOption = listCommand.getOptions().find(
        (opt) => opt.name === "live-stream-id"
      );
