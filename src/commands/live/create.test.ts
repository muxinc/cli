...
describe("mux live create command", () => {
  ...
  describe("Optional flags", () => {
    test("has --playback-policy flag", () => {
      const playbackPolicyOption = createCommand.getOptions().find(
    ...
    test("has --new-asset-settings flag", () => {
      const newAssetOption = createCommand.getOptions().find(
        (opt) => opt.name === "new-asset-settings"
      );
      expect(newAssetOption).toBeDefined();
    });

    test("has --reconnect-window flag", () => {
      const reconnectOption = createCommand.getOptions().find(
        (opt) => opt.name === "reconnect-window"
      );
      expect(reconnectOption).toBeDefined();
    });

    test("has --latency-mode flag", () => {
      const latencyOption = createCommand.getOptions().find(
        (opt) => opt.name === "latency-mode"
      );
