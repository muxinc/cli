...
All OAuth endpoints live on the Mux API host, so they are derived from one base — `MUX_BASE_URL` moves the API calls, discovery, and the sign-in flow together, and the token endpoint can never end up on a different host than the authorization endpoint. A login made against a non-default host stores that host with the environment, so its API calls, token refresh, and revocation keep going there in shells where `MUX_BASE_URL` is not set. On top of that, the CLI discovers the authorization server's endpoints at runtime (RFC 8414 / OIDC discovery, cached for a day in `~/.cache/mux/`), so Mux can move them without breaking installed versions. Discovery is never required: if it is unreachable the CLI falls back to built-in defaults, and `MUX_OAUTH_AUTHORIZE_URL` / `MUX_OAUTH_TOKEN_URL` / `MUX_OAUTH_REVOKE_URL` always win when set. It is consulted only on login, refresh, and logout (to revoke), never on ordinary commands.
